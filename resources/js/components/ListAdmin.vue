    ...
    <div>
        <div class="table-container">
            <table>
                ...
                <tr v-for="(admin, index) in adminsLocal">
                    <td>
                        <div v-if="!admin.editing">{{admin.name}}</div>
                        <div v-else class="row middle-items FormEdit">
                            <input class="col-10" type="text" v-model="admin.name">
                        </div>

                    </td>
                    <td>
                        <div v-if="!admin.editing">{{admin.email}}</div>
                        <div v-else class="row middle-items FormEdit">
                            <input class="col-10" type="text" v-model="admin.email">
                        </div>
                    </td>
                    <td  v-if="!admin.editing"  class="row justify-center">
                        <a href="" @click.prevent="editAdmin(admin)">
                    ...
                    <td v-else class="row justify-center">
                        <div class="m-l-4  is-text-center">
                            <a :class="[admin.updating ? 'disabled' : '', 'FormEdit-link' ] "
                               @click.prevent="updateAdmin(admin)">✓</a>
                        </div>
                        <div class="m-l-4   is-text-center">
                            <a class="FormEdit-link cancel" @click.prevent="cancelAdmin(admin)">X</a>
                        </div>
                    </td>
                </tr>
...
<script>
    ...
    export default {
        ...
        mounted: function () {
            this.adminsLocal = Object.assign({}, this.admins, this.admins.map((i) => (
                {'id': i.id, 'name': i.name, 'email': i.email, 'editing': false, 'updating': false}
            )));
        ...
        methods: {
            ...
            updateAdmin(admin) {
                ajax.put(`/admin/usuarios-Admin/${admin.id}`, admin)
                    .then(() => admin.editing = false)
                    .catch(() => {
                        admin.editing = false;
